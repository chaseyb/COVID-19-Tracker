...
}); // MapBox implementation by Chase & Maurice

/* BEGIN MAPBOX */ // the base url end point
var coronaData = {
	method: 'GET',
	redirect: 'follow'
};
...
fetch("https://corona.lmao.ninja/v2/jhucsse")
	...
	.then(coronaData => {
		const { country, stats, coordinates } = coronaData;
...
mapboxgl.accessToken =
	'pk.eyJ1IjoiY2hhc2V5YiIsImEiOiJja2EydmhiMXIwM2Y1M2xzNW5oMnRpYzd5In0.m1vDX_9oLA_Ywa2fa43WXg';
		...
		map.addSource('places', {
			...
			data: {
				...
				features: await Promise.all(
					coronaData.stats.map(async (country) => {
...
async function performAsyncCall() {
	const response = await fetch(`https://corona.lmao.ninja/v2/jhucsse`);
	const data = await response.json();
	const { country, countryInfo } = data;
	coronaData.countryInfo = countryInfo;
}
	...
	const locationCoronaDetails = coronaData.locations.reduce(
		(accumulator, currentLocation) => {
			if (currentLocation.country === countrySelected) {
				...
				accumulator['ios2'] = currentLocation.country_code;
...
/* BEGIN NEWS, TESTING & STATS CARDS */ // Global var used in ajax calls and Select State Dropdown
let userState = '';
...
$.ajax(settings).done(function (response) {
	...
	$('#global-cases').append(
		"<p class='cards-p'>Active:</p><p class='cards-p'>" + globalActive + '</p>'
	);
	$('#global-recovered').append(
		"<p class='cards-p'>Recovered:</p><p class='cards-p'>" +
		globalRecovered +
		'</p>'
	);
	$('#global-deaths').append(
		"<p class='cards-p'>Deceased:</p><p class='cards-p'>" +
		globalDeaths +
		'</p>'
	);
	$('#global-new').append(
		"<p class='cards-p'>New Today:</p><p class='cards-p'>" +
		globalToday +
		'</p>'
	);
...
function stateStatsCall() {
	...
	$.ajax(settings).done(function (response) {
		...
		// append to Data Cards, with hr above p's
		$('#state-name').append(
			"<p class='cards-p wipe'>State:</p><p class='cards-p wipe'>" +
			stateName +
			'</p>'
		);
		$('#state-cases').append(
			"<p class='cards-p wipe'>Active:</p><p class='cards-p wipe'>" +
			stateActive +
			'</p>'
		);
		$('#state-deaths').append(
			"<p class='cards-p wipe'>Deceased:</p><p class='cards-p wipe'>" +
			stateDeaths +
			'</p>'
		);
		$('#state-new').append(
			"<p class='cards-p wipe'>New Today:</p><p class='cards-p wipe'>" +
			stateToday +
			'</p>'
		);
...
function ajaxNews() {
	...
	let newsURL =
		`https://api.nytimes.com/svc/search/v2/articlesearch.json?q=coronavirus&` +
	...
	$.ajax({
		...
		for (i = 0; i < response.length; i++) {
			...
			if (response[i].name === 'No Locations Yet') {
				$('#test-list').append(
					"<li style='font-weight: 500'>No current" +
